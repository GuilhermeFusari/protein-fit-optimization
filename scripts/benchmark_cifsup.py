...
Uso:
    ...
    # salvando as poses do cifsup para calculo posterior de NSD:
    python3 benchmark_cifsup.py ... --save-aligned poses_cifsup
...
def main():
    ...
    ap.add_argument("--save-aligned", default="",
                    help="pasta para salvar as poses do cifsup (NSD e ICP)")
    args = ap.parse_args()
    ...
    for i, acc in enumerate(entries, 1):
        ...
        try:
            ...
            write_pdb(read_coords(env_src), env_pdb)
                ...
                if st == "ok":
                    ...
                    if len(fit):
                        ...
                        if args.save_aligned:
                            os.makedirs(args.save_aligned, exist_ok=True)
                            write_pdb(fit, os.path.join(args.save_aligned,
                                                        f"{acc}_{k}.pdb"))
            if acc in ours:
                o = ours[acc]
