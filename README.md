...
|---|---|---|---|---|
| Unconstrained ICP (baseline) | 5.912 | 0.169 | 0.493 | n/a |
...
1 to 200 shows monotonic degradation, and λ = 2 gives the best balance.
...
the time. In this benchmark the mirrored hand won in **26 of 50** entries,
...
silently. This was the cause of the two failures originally reported in the
benchmark (SASDXB2 and SASDXS4), which in fact align normally.
...
│  ├─ gerar_figuras_artigo.py      figures 1 to 4 and tables
...
* Packing shows the envelope accommodates more than one rigid conformer, which
  is consistent with both oligomerisation and a conformational ensemble.
  Distinguishing them requires molecular weight from I(0).
